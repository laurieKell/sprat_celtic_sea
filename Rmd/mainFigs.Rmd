...
f=FLQuant(c(computeRefpts(eq4)["msy","harvest"]),dimnames=list(year=dimnames(om4)$year,season=1:4)) 
f[,,,1:2]=1e-6 
f[,,,3]  =f[,,,3]*0.25
f[,,,4]  =f[,,,4]*0.75
...
# Operating Model Simulations for estimating classification skill
...
p1=plot(window(FLStocks("Step"=kick[[1]],"Trend"=ftrend[[1]]),start=2010,end=2031),metrics=list(
    ...
    theme_bw()+
    scale_fill_manual(  "Scenario",values=c("cyan","red"))+
    scale_colour_manual("Scenario",values=c("cyan","red"))+
...
```{r oem-plot, fig.height=0.75*10, fig.width = 8}
p2=plot(window(FLStocks("Step"=kick[[1]],"Trend"=ftrend[[1]]),start=2010,end=2031),metrics=list(
    LBI  =function(x) {res=indicators.len(x[,,,3], indicators=c('lmean'), params=pars[,1], metric='stock.n')[[1]] 
                      names(dimnames(res))[1]="age"
                      res},
    Index=function(x) rlnorm(dim(x)[6],log(biomass(x[,,,3])),0.3), 
    HR   =function(x) apply(catch(x),c(2.6),sum)%/%vb(x)[,,,3]
    ))+
    theme_bw()+
    theme(legend.position="bottom")+
    ...
    scale_fill_manual(  "Scenario",values=c("cyan","red"))+
    scale_colour_manual("Scenario",values=c("cyan","red"))+
    scale_y_continuous(limit=c(11.5,NA)) 
p2 
```

**Figure `r iFig=iFig+1; iFig`** OEM 

```{r, oem, fig.height=6, fig.width=10}
ggarrange(p1,p2,
          widths = c(4), heights = c(1,1,1),
          nrow   = 1,          ncol    = 2,
          labels = c("OM","OEM"), label.x=c(0.15,0.15), label.y=c(0.98),
          common.legend = TRUE,
...
p<-function() plot(iter(window(FLStocks("1"=om1,"4"=om4,"6"=om6,"7"=om7,"8"=om8),
  ...
  geom_flpar(data=FLPars(SSB   =FLPar("MSY"=refpts(eq4)[c("msy"),"ssb",1,drop=T]),
             ...
             label=rep(c("MSY",rep(NA,4)),each=4),alpha=0.5)+
  geom_flpar(data=FLPars(Recruits=FLPar("Virgin"=refpts(eq4)[c("virgin"),"rec",1,drop=T]), 
                         ...
                         Forage=FLPar("Virgin"=23314)),x=ISOdate(2012,1,1),linetype=3,label=rep(c("Virgin",rep(NA,4)),each=3),alpha=0.5)+
...
```{r, ts-3-prj, fig.height=0.9*8, fig.width=10,eval=FALSE}
i=4 
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=om[[i]]
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=om[[i]]
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=om[[i]]
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=om[[i]]
load(paste("../data/runs/mse.10.17.10.17.TRUE",i,"RData",sep="."));om5=om[[i]]
load(paste("../data/runs/mse.13.57.13.57.TRUE",i,"RData",sep="."));om6=om[[i]]
load(paste("../data/runs/mse.27.13.27.13.TRUE",i,"RData",sep="."));om7=om[[i]]
load(paste("../data/runs/mse.40.7.40.7.TRUE",i,"RData",sep="."));om8=om[[i]]
...
```{r, ts-3-hr1, fig.height=0.9*8, fig.width=10, eval=FALSE}
i=4
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=HR1 
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=HR1 
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=HR1 
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=HR1 
load(paste("../data/runs/mse.10.17.10.17.TRUE",i,"RData",sep="."));om5=HR1 
load(paste("../data/runs/mse.13.57.13.57.TRUE",i,"RData",sep="."));om6=HR1 
load(paste("../data/runs/mse.27.13.27.13.TRUE",i,"RData",sep="."));om7=HR1
load(paste("../data/runs/mse.40.7.40.7.TRUE",i,"RData",sep="."));om8=HR1 
...
```{r, ts-3-hr0, fig.height=0.9*8, fig.width=10, eval=FALSE}
i=4
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=HR0
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=HR0
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=HR0
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=HR0 
load(paste("../data/runs/mse.10.17.10.17.TRUE",i,"RData",sep="."));om5=HR0
load(paste("../data/runs/mse.13.57.13.57.TRUE",i,"RData",sep="."));om6=HR0
load(paste("../data/runs/mse.27.13.27.13.TRUE",i,"RData",sep="."));om7=HR0
load(paste("../data/runs/mse.40.7.40.7.TRUE",i,"RData",sep="."));om8=HR0 
...
```{r, ts-3-xy1, fig.height=0.9*8, fig.width=10, eval=FALSE}
i=4
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=XY1 
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=XY1
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=XY1 
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=XY1 
load(paste("../data/runs/mse.10.17.10.17.TRUE",i,"RData",sep="."));om5=XY1 
load(paste("../data/runs/mse.13.57.13.57.TRUE",i,"RData",sep="."));om6=XY1 
load(paste("../data/runs/mse.27.13.27.13.TRUE",i,"RData",sep="."));om7=XY1 
load(paste("../data/runs/mse.40.7.40.7.TRUE",i,"RData",sep="."));om8=XY1 
...
```{r, ts-3-xy0, fig.height=0.9*8, fig.width=10, eval=FALSE}
i=4
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=XY0 
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=XY0
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=XY0 
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=XY0 
load(paste("../data/runs/mse.10.17.10.17.TRUE",i,"RData",sep="."));om5=XY0 
load(paste("../data/runs/mse.13.57.13.57.TRUE",i,"RData",sep="."));om6=XY0 
load(paste("../data/runs/mse.27.13.27.13.TRUE",i,"RData",sep="."));om7=XY0 
load(paste("../data/runs/mse.40.7.40.7.TRUE",i,"RData",sep="."));om8=XY0 

p() 
rp=rbind(rp,cbind("What"="XY0",p2()))

#save(rp,file="../data/results/msyOM.RData")
```

**Figure `r iFig=iFig+1; iFig`**  HCR, 2 over 3, with 0 year lag.

```{r, bxplt-3, fig.height=0.75*10, eval=FALSE}
#load("../data/results/msyOM.RData") 
#load("../data/om/eq4M2.RData")
...
ggplot(subset(dat,What%in%c("projection","XY1","HR0")))+
  ...
  facet_grid(qname~factor(.id,labels=c("2.5% Fmsy", "25% Fmsy", "50% Fmsy", "Fmsy", "150% Fmsy")),scale="free")+
   geom_flpar(data=FLPars(SSB   =FLPar("MSY"=refpts(eq4)[c("msy"),"ssb",1,drop=T]),
             ...
             x=1,linetype=2,label=NA)+
  geom_flpar(data=FLPars(Rec =FLPar("Virgin"=refpts(eq4)[c("virgin"),"rec",1,drop=T]), 
                         SSB=FLPar("Virgin"=refpts(eq4)[c("virgin"),"ssb",1,drop=T]),
                         Forage=FLPar("Virgin"=23314)),x=1,linetype=1,label=NA)+
  theme_bw(16)+
  theme(legend.position="bottom",
...
```{r, fig7a, fig.height=0.75*10,fig.width=10, eval=FALSE}
load("../data/results/msyOM.RData")
load("../data/om/eq4.RData") 

dat=transform(subset(rp,.id%in%c(1,4,6,7,8) & year%in%2021:2030),
              What=factor(What,levels=unique(rp$What)[c(1,4,5,2,3)],labels=c("Projection","XY1","XY0","HR1","HR0")),
              qname=factor(qname,levels=c("F","SSB","Rec","Catch","Forage"),labels=c("F","SSB","Recruits","Catch","Forage")))
dat=ddply(dat,.(.id,qname,What), with, data.frame(data=filterLims(data)))
...
```{r, ts-4-prj, fig.height=0.9*8, fig.width=10, eval=FALSE}
i=4
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=om[[i]]
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=om[[i]]
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=om[[i]]
...
```{r, ts-4-hr1, fig.height=0.9*8, fig.width=10, eval=FALSE}
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=HR1 
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=HR1 
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=HR1 
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=HR1 
...
```{r, ts-4-hr0, fig.height=0.9*8, fig.width=10, eval=FALSE}
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=HR0
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=HR0
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=HR0
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=HR0 
...
```{r, ts-4-xy1,  fig.height=0.9*8, fig.width=10, eval=FALSE}
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=XY1 
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=XY1
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=XY1 
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=XY1 
...
```{r, ts-4-xy0,  fig.height=0.9*8, fig.width=10, eval=FALSE}
load(paste("../data/runs/mse.0.68.0.68.TRUE",i,"RData",sep="."));om1=XY0 
load(paste("../data/runs/mse.1.7.1.7.TRUE",i,"RData",sep="."));om2=XY0
load(paste("../data/runs/mse.3.39.3.39.TRUE",i,"RData",sep="."));om3=XY0 
load(paste("../data/runs/mse.6.78.6.78.TRUE",i,"RData",sep="."));om4=XY0 
...
```{r, fig7b, fig.height=0.75*10,fig.width=10, eval=FALSE}
...
**Figure `r iFig=iFig+1; iFig`**  Summary A


```{r, fig7c, fig.height=0.75*10,fig.width=10,eval=FALSE}
...
a1=ggplot(subset(rocIdx,Scenario=="M2"))+
  ...
  geom_text(aes(x=x,y=y,label=label),data=data.frame(x=c(0.3,0.3,2.3,2.3),y=c(0.2,5,5,0.2),label=c("TN","FP","TP","FN")),
            size=6,col="blue")+
  theme_bw(16)+
...
a2=ggplot(subset(rocLbi,Scenario=="M2"))+
  ...
  geom_hline(aes(yintercept=1),col="red",linetype=2)+
  geom_vline(aes(xintercept=1),col="red",linetype=2)+
  geom_text(aes(x=x,y=y,label=label),data=data.frame(x=c(0.2,0.2,3.3,3.3),y=c(0.97,1.085,1.085,0.97),label=c("TN","FP","TP","FN")),
            size=6,col="blue")+
  theme_bw(16)+
  xlab("Operating Model")+
  ylab("LBI")
...
a3=ggplot(subset(rocHr,Scenario=="M2"&year==2025&MP<5))+
  ...
  geom_hline(aes(yintercept=1),col="red",linetype=2)+
  geom_vline(aes(xintercept=1),col="red",linetype=2)+
  geom_text(aes(x=x,y=y,label=label),data=data.frame(x=c(0.25,0.25,3.25,3.25),y=c(0.5,4.75,4.75,0.5),label=c("TN","FP","TP","FN")),
            size=6,col="blue")+
  theme_bw(16)+
  xlab("Operating Model")+
  ylab("Harvest Rate")
  
ggarrange(a1, a2, a3, nrow=1, ncol=3, common.legend=TRUE, labels=c("a)","b)","c)")) 
```

**Figure `r iFig=iFig+1; iFig`**  C

```{r, rocs, fig.height=0.7*4.75, fig.width=0.75*12}
load("../data/results/roc.RData")

rocIdx=transform(rocIdx,Lag=year-2020)
rocLbi=transform(rocLbi,Lag=year-2020)
rocHr =transform(rocHr, Lag=year-2020)

b1=ggplot(transform(subset(rocIdx,Scenario=="M Cohort"),Lag=ac(Lag)))+
  geom_path( aes( FPR,TPR,col=Lag))+
...
b2=ggplot(transform(subset(rocLbi,Scenario=="M Cohort"),Lag=ac(Lag)))+
  geom_path( aes( FPR,TPR,col=Lag))+
  geom_line(aes(x,y),data.frame(x=seq(0,1,0.01),y=seq(0,1,0.01)))+
  theme_bw(16)+
  theme(legend.position="none")+
...
b3=ggplot(transform(subset(rocHr,Scenario=="M Cohort"),Lag=ac(Lag)))+
  geom_path( aes( FPR,TPR,col=Lag))+
  geom_line(aes(x,y),data.frame(x=seq(0,1,0.01),y=seq(0,1,0.01)))+
  theme_bw(16)+
  theme(legend.position="none")+
...
ggarrange(b1, b2, b3, nrow=1, ncol=3, common.legend = TRUE, legend="bottom", labels=c("a)","b)","c)"))  
```

**Figure `r iFig=iFig+1; iFig`**  D

doi:10.1016/j.icesjms.2006.04.015

https://doi.org/10.1016/j.fishres.2021.105952
...
where $N$ is numbers at length $L$ and $\Omega$ is a prefactor and $\kappa$ the scaling by length – and assuming that predation by all fish in the spectrum is mediated through a feeding relationship with mean $\mu$ and standard deviation $\sigma$, both common to all predators - and that $UM2$ is a constant that relates predation pressure to mortality and where λ is the scaling of the feeding rate of predators by length - then the predation mortality on a prey species of length L′ can be written as
