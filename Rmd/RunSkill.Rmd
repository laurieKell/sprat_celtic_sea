...
```{r}
object=FLStocks("SQ"=burnin[[3]],"Predators"=burnin[[4]],"Kick"=kick[[4]],"No demand"=nodemand[[4]])
...
```{r}
save(burnin,kick,nodemand,eq4,pars,sr4,file=file.path(dirOM,"om-skill.RData"))
```
...
idx=mdply(data.frame(OM=names(oms)), function(OM) ldply(oms[[OM]], function(x) 
                      as.data.frame(biomass(x)[,,,3]%*%hDevs,drop=T)))

smry=cbind(om,lbi=lbi[,"data"],idx=idx[,"data"])

save(smry,file=file.path(dirRes,"smry.RData"))
```

```{r}
save(hDevs,file=file.path(dirOM,"dev-idx.RData"))
```
...
load(file.path(dirRes,"smry.RData"))
